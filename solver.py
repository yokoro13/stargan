...
class Solver(object):
    ...
    def create_labels(self, c_org, c_dim=5, dataset='CelebA', selected_attrs=None):
        ...
        if dataset == 'CelebA':
            ...
            for i, attr_name in enumerate(selected_attrs):
                if attr_name in ['black_floor', 'white_floor', 'wood_floor', "ocean"]:
                    floor_color_indices.append(i)
                if attr_name in ['red_brick_wall', 'white_wall', 'wood_wall', "sky"]:
                    wall_color_indices.append(i)

        c_trg_list = []
        for i in range(c_dim):
            if dataset == 'CelebA':
                c_trg = c_org.clone()
                if i in floor_color_indices:  # Set one hair color to 1 and the rest to 0.
                    c_trg[:, i] = 1
                    for j in floor_color_indices:
                        if j != i:
                            c_trg[:, j] = 0

                elif i in wall_color_indices:  # Set one hair color to 1 and the rest to 0.
                    c_trg[:, i] = 1
                    for j in wall_color_indices:
                        if j != i:
                            c_trg[:, j] = 0
                else:
    ...
    def train_multi(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            # Save model checkpoints.
            if (i + 1) % self.model_save_step == 0:
                G_path = os.path.join(self.model_save_dir, '{}-G_256.ckpt'.format(i + 1))
                D_path = os.path.join(self.model_save_dir, '{}-D_256.ckpt'.format(i + 1))
                torch.save(self.G.state_dict(), G_path)
                torch.save(self.D.state_dict(), D_path)
                print('Saved model checkpoints into {}...'.format(self.model_save_dir))
